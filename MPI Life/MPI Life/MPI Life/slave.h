...
void MPI_Slave(int n,int m, int tm, int th, int rank, MPI_Status* status) {
    ...
    for(int t = 0; t < tm; ++t) {
        for(int i = 1; i <= bounds.second - bounds.first; ++i) {
            calc_next(&field_part[cur_part], &field_part[1-cur_part], i, bounds.second - bounds.first + 2, m);
        }
        
        cur_part = 1 - cur_part;
        
        MPI_Send(&field_part[cur_part][1][0], m, MPI_CHAR, prev, 3, MPI_COMM_WORLD);
        MPI_Send(&field_part[cur_part][bounds.second - bounds.first][0], m, MPI_CHAR, next, 4, MPI_COMM_WORLD);
        
        
        MPI_Recv(&field_part[cur_part][0][0], m, MPI_CHAR, prev, 4, MPI_COMM_WORLD, status);
        MPI_Recv(&field_part[cur_part][bounds.second - bounds.first + 1][0], m, MPI_CHAR, next, 3, MPI_COMM_WORLD, status);
    }
